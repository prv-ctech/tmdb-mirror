...
db_stats="$(psql_at "$password" "SELECT json_build_object(
  ...
  'final_image_bytes', (SELECT COALESCE(sum(file_size_bytes), 0) FROM assets.image_assets WHERE status = 'ready'),
  'invalid_legacy_paths', (SELECT count(*) FROM assets.image_assets WHERE storage_path ~ '(^|/)(optimized|\.masters)(/|$)'),
  'media_requests_by_status', (SELECT COALESCE(json_object_agg(status, request_count ORDER BY status), '{}'::json) FROM (SELECT status, count(*) AS request_count FROM ops.media_requests GROUP BY status) request_counts),
  'video_counts_by_type_and_site', (SELECT COALESCE(json_object_agg(video_type || '/' || site, video_count ORDER BY video_type, site), '{}'::json) FROM (SELECT COALESCE(video_type, 'unknown') AS video_type, site, count(*) AS video_count FROM catalog.title_videos GROUP BY COALESCE(video_type, 'unknown'), site) video_counts),
