...
db_stats="$(psql_at "$password" "SELECT json_build_object(
  ...
  'downloaded_original_rows', (SELECT count(*) FROM assets.image_assets WHERE status = 'ready' AND source_storage_path IS NOT NULL),
  'optimized_asset_rows', (SELECT count(*) FROM assets.image_assets WHERE status = 'ready' AND storage_path ~ '(^|/)optimized/'),
  'optimized_variant_rows', (SELECT count(*) FROM assets.image_variants),
