-- Serialize bounded queue-capacity checks with one lock. A catalog
-- transaction may enqueue title, season, and image jobs in one unit of work;
-- per-queue locks allowed those transactions to wait on each other forever.
...
BEGIN
    ...
    THEN
        -- The application also takes this lock while checking a batch before
        -- inserting it. Keep the database trigger on the same lock so every
        -- enqueue path has one ordering and cannot form a lock cycle.
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0043'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 43
  AND (
