-- Typed TMDB trending refreshes are durable ingestion work. The worker owns
-- the fixed payload shape; no operator-supplied SQL, path, or URL is accepted.

INSERT INTO ops.job_type_registry (job_type, payload_version, enabled)
VALUES ('ingest.trending', 1, true)
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0024'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 24
  AND (
