-- The public TMDB v3 state endpoints write through api_job_submitter. Table
-- privileges alone are insufficient without access to their owning schema.

GRANT USAGE ON SCHEMA source TO api_job_submitter;
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0050'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 50
  AND (
