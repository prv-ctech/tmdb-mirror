-- The ingest worker reads assets while selecting missing media work.

GRANT USAGE ON SCHEMA assets TO ingest_writer;
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0039'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 39
  AND (
