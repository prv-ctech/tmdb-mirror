-- Title census and deep enrichment advance independently. Both remain
-- bounded, but a full enrichment queue must not stall the next ID batch.
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0044'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 44
  AND (
