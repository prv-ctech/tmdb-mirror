-- The ingest worker uses this read-only count to enforce queue bounds.
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0038'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 38
  AND (
