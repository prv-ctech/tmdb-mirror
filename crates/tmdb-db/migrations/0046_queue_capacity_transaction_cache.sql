-- Preserve the global queue cap without recounting the same queue for every
-- row inserted by one transaction. The advisory lock remains held until the
-- transaction ends, so the cached count can be advanced safely per new row.
...
BEGIN
    ...
    max_active_jobs := CASE queue_name
        ...
        WHEN 'title.enrichment' THEN 1000
    ...
    THEN
        cached_queue := pg_catalog.current_setting(
            'tmdb.queue_capacity_name', true
        );
        cached_count := pg_catalog.current_setting(
            'tmdb.queue_capacity_count', true
        );
        cache_is_valid := cached_queue = queue_name
            AND cached_count ~ '^[0-9]+$';

        IF NOT cache_is_valid THEN
        ...
        IF cache_is_valid THEN
            active_jobs := cached_count::bigint;
        ELSE
            SELECT pg_catalog.count(*)
              INTO active_jobs
              FROM ops.jobs AS existing
             WHERE (
                    (queue_name = 'image.download'
                        AND existing.job_type = 'image.download')
                    OR (queue_name = 'title.refresh'
                        AND existing.job_type IN (
                            'ingest.refresh_movie', 'ingest.refresh_tv'
                        ))
                    OR (queue_name = 'title.enrichment'
                        AND existing.job_type IN (
        ...
        PERFORM pg_catalog.set_config(
            'tmdb.queue_capacity_name', queue_name, true
        );
        PERFORM pg_catalog.set_config(
            'tmdb.queue_capacity_count', (active_jobs + 1)::text, true
        );
    END IF;
    RETURN NEW;
END
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0046'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 46
  AND (
