-- Catalog and media work is administrator-triggered. Remove the obsolete
-- scheduler checkpoint table so an older worker cannot create automatic work.
DROP TABLE IF EXISTS ops.scheduler_runs;
...
WHERE metadata.key = 'schema'
  AND metadata.value ->> 'revision' = '0040'
  AND (SELECT pg_catalog.count(*) FROM ops._sqlx_migrations) = 40
  AND (
