...
impl AdminApiStore for FakeAdminStore {
    async fn status(&self) -> Result<AdminStatus, AdminApiError> {
        Ok(AdminStatus {
            build: AdminBuildStatus {
                ...
                schema_revision: Some("0026".to_owned()),
            },
    ...
    async fn submit(
        ...
        assert_eq!(idempotency_key, "scan-1");
        assert!(matches!(operation, AdminOperation::Scan { .. }));
        Ok(AdminSubmission {
            job_id: self.job_id,
            duplicate: false,
        })
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(body["data"]["build"]["schemaRevision"], "0026");
...
async fn operation_payloads_and_keys_are_bounded() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = router
        .oneshot(
            Request::post("/admin/v1/media/audits")
                .header("x-api-key", "a-test-key-that-is-long-enough-to-be-valid")
                .header("idempotency-key", "x".repeat(129))
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(r#"{"repair":true}"#))?,
        )
        .await?;
    assert_eq!(response.status(), StatusCode::BAD_REQUEST);
    ...
    let response = router
        .oneshot(
            Request::post("/admin/v1/media/audits")
                .header("x-api-key", "a-test-key-that-is-long-enough-to-be-valid")
                .header("idempotency-key", "audit-1")
                .header(header::CONTENT_TYPE, "application/json")
